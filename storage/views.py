...
def delete_item(request, item_type, item_id):
    """Delete file or folder and update storage usage"""
    if item_type == 'file':
        ...
        # Update user's storage usage (subtract the file size)
        profile = request.user.profile
        ...
        # Calculate total size of folder contents before deletion
        ...
        def delete_folder_contents(folder):
            ...
            for subfolder in folder.subfolders.all():
                folder_size += delete_folder_contents(subfolder)
                subfolder.delete()
            
            return folder_size
        
        # Delete contents and get total size deleted
        deleted_size = delete_folder_contents(item)
        
        # Update user's storage usage
        profile = request.user.profile
        profile.storage_used = max(0, profile.storage_used - deleted_size)
        profile.save()
...
def view_file(request, file_id):
    ...
    # Create response with inline content disposition to open in browser
...
def update_storage_on_file_delete(sender, instance, **kwargs):
    """Update user storage when a file is deleted"""
    try:
        profile = instance.owner.profile
        profile.storage_used = max(0, profile.storage_used - instance.size)
        profile.save()
    except:
